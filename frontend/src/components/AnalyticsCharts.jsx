...
export const DemandDistributionChart = ({ data, darkMode }) => {
  ...
  const min = Math.min(...data.map(d => d.predicted_demand || d.target_demand || 0))
  const max = Math.max(...data.map(d => d.predicted_demand || d.target_demand || 0))
    ...
    <ChartContainer title="Demand Distribution" darkMode={darkMode}>
      <ResponsiveContainer width="100%" height={300}>
        <BarChart data={bins}>
          ...
          <XAxis dataKey="range" stroke={darkMode ? '#9ca3af' : '#6b7280'} angle={-45} textAnchor="end" height={100} />
    ...
    <ChartContainer title="Category Performance" darkMode={darkMode}>
      <ResponsiveContainer width="100%" height={300}>
        <BarChart data={chartData}>
          ...
          <Tooltip contentStyle={tooltipStyle(darkMode)} />
          <Legend />
          <Bar dataKey="avgDemand" fill="#3b82f6" radius={[8, 8, 0, 0]} name="Avg Demand" />
...
export const StorePerformanceChart = ({ data, darkMode }) => {
  ...
  const chartData = Object.values(storeMap).map(store => ({
    ...store,
  ...
  return (
    <ChartContainer title="Top 10 Store Performance" darkMode={darkMode}>
      <ResponsiveContainer width="100%" height={300}>
        <BarChart data={chartData}>
          <CartesianGrid strokeDasharray="3 3" stroke={darkMode ? '#374151' : '#e5e7eb'} />
          <XAxis dataKey="store" stroke={darkMode ? '#9ca3af' : '#6b7280'} angle={-45} textAnchor="end" height={100} />
          <YAxis stroke={darkMode ? '#9ca3af' : '#6b7280'} />
          <Tooltip contentStyle={tooltipStyle(darkMode)} />
          <Legend />
          <Bar dataKey="avgRevenue" fill="#10b981" radius={[8, 8, 0, 0]} name="Avg Revenue" />
        </BarChart>
      </ResponsiveContainer>
    </ChartContainer>
  )
...
export const ProductPerformanceChart = ({ data, topProducts, darkMode }) => {
      ...
      <ChartContainer title="Top 10 Products by Demand" darkMode={darkMode}>
        <ResponsiveContainer width="100%" height={300}>
          <ScatterChart margin={{ top: 20, right: 20, bottom: 20, left: 20 }}>
            <CartesianGrid strokeDasharray="3 3" stroke={darkMode ? '#374151' : '#e5e7eb'} />
            <XAxis type="number" dataKey="price" stroke={darkMode ? '#9ca3af' : '#6b7280'} name="Price" label={{ value: 'Price', position: 'insideBottom', offset: -10 }} />
            <YAxis type="number" stroke={darkMode ? '#9ca3af' : '#6b7280'} name="Demand" label={{ value: 'Demand', angle: -90, position: 'insideLeft' }} />
            <Tooltip contentStyle={tooltipStyle(darkMode)} cursor={{ strokeDasharray: '3 3' }} />
  ...
  const chartData = Object.values(productMap).map(prod => ({
    ...prod,
    ...
    <ChartContainer title="Price vs Demand (Elasticity)" darkMode={darkMode}>
      <ResponsiveContainer width="100%" height={300}>
        <LineChart data={chartData}>
          ...
          <Tooltip contentStyle={tooltipStyle(darkMode)} />
          <Legend />
          <Line type="monotone" dataKey="demand" stroke="#ef4444" dot={false} name="Demand" strokeWidth={2} />
        </LineChart>
      </ResponsiveContainer>
    </ChartContainer>
  )
...
export const HolidayImpactChart = ({ data, darkMode }) => {
  ...
  const avgWithHoliday = withHoliday.length > 0 
    ? (withHoliday.reduce((sum, d) => sum + (d.predicted_demand || d.target_demand || 0), 0) / withHoliday.length).toFixed(2)
    : 0

  const avgNoHoliday = noHoliday.length > 0
    ? (noHoliday.reduce((sum, d) => sum + (d.predicted_demand || d.target_demand || 0), 0) / noHoliday.length).toFixed(2)
    : 0

  const chartData = [
    { name: 'Regular Days', demand: parseFloat(avgNoHoliday), count: noHoliday.length },
    { name: 'Holiday', demand: parseFloat(avgWithHoliday), count: withHoliday.length },
  ]

  const boost = ((avgWithHoliday - avgNoHoliday) / avgNoHoliday * 100).toFixed(2)
  ...
  return (
    <ChartContainer title="Holiday Impact on Demand" darkMode={darkMode}>
      ...
      <div className={`mt-4 p-4 rounded-lg ${darkMode ? 'bg-gray-700' : 'bg-amber-50'}`}>
        <p className={`text-sm ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>
          <span className="font-semibold">Holiday Boost: </span>{boost}%
        </p>
      </div>
    </ChartContainer>
  )
...
export const EconomicIndexChart = ({ data, darkMode }) => {
  ...
  const sorted = [...data].sort((a, b) => (a.economic_index || 0) - (b.economic_index || 0))
  ...
  return (
    <ChartContainer title="Economic Index Impact on Demand" darkMode={darkMode}>
      <ResponsiveContainer width="100%" height={300}>
        <ScatterChart margin={{ top: 20, right: 20, bottom: 20, left: 20 }}>
          <CartesianGrid strokeDasharray="3 3" stroke={darkMode ? '#374151' : '#e5e7eb'} />
          <XAxis type="number" dataKey="economicIndex" stroke={darkMode ? '#9ca3af' : '#6b7280'} name="Economic Index" label={{ value: 'Economic Index', position: 'insideBottom', offset: -10 }} />
          <YAxis type="number" stroke={darkMode ? '#9ca3af' : '#6b7280'} name="Demand" label={{ value: 'Demand', angle: -90, position: 'insideLeft' }} />
          <Tooltip contentStyle={tooltipStyle(darkMode)} />
...
export const ConfidenceDistributionChart = ({ data, darkMode }) => {
  ...
  const medium = confidenceData.filter(d => d.confidence >= 0.6 && d.confidence < 0.8).length
  ...
  return (
    <ChartContainer title="Prediction Confidence Distribution" darkMode={darkMode}>
      <ResponsiveContainer width="100%" height={300}>
        <PieChart>
          <Pie
            data={chartData}
