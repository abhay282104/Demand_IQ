...
async def predict(
    ...
    if not prediction_service:
        raise HTTPException(
            status_code=503,
            detail="Model not loaded. Server still initializing."
        )
    
    try:
        response = prediction_service.predict(request, db)
        return response
...
async def predict_batch(
    ...
    try:
        ...
        required_columns = [
            'date', 'product_id', 'category_id', 'store_id',
            'historical_sales', 'price', 'promotion_flag',
            'holiday_flag', 'economic_index'
        ]
        ...
        for idx, row in df.iterrows():
            try:
                ...
                pred = prediction_service.predict(request, db)
        ...
        # Calculate analytics
        successful_predictions = [p for p in predictions_list if 'predicted_demand' in p]
        ...
        if successful_predictions:
            predictions_array = np.array([p['predicted_demand'] for p in successful_predictions])
            confidence_array = np.array([p['confidence'] for p in successful_predictions])
            ...
            actual_demands = [p.get('actual_demand') for p in successful_predictions if 'actual_demand' in p and p['actual_demand'] is not None]
            ...
            if actual_demands:
                ...
                analytics.update({
                    ...
                    'mape': float(np.mean(np.abs(errors / (actual_array + 1))) * 100),
                })
...
async def train_model(
    ...
    if DemandForecastingPipeline is None:
        raise HTTPException(
            status_code=500,
            detail="Training pipeline not available."
        )
    ...
    try:
        ...
        required_columns = [
            'date', 'product_id', 'category_id', 'store_id',
            'historical_sales', 'price', 'promotion_flag',
            'holiday_flag', 'economic_index', 'target_demand',
        ]
        ...
        for idx, row in df.iterrows():
            try:
                ...
                pred = prediction_service.predict(req, db)
                predictions_list.append({
                    'row_index': idx,
                    'date': row.get('date'),
                    'product_id': int(row['product_id']),
                ...
                logger.error(f"Error predicting row {idx} during train analysis: {str(e)}")
                predictions_list.append({
                    'row_index': idx,
                    'error': str(e)
                })
        ...
        analytics = {
            'total_rows': len(df),
            'successful_predictions': len([p for p in predictions_list if 'predicted_demand' in p]),
            'failed_predictions': len(predictions_list) - len([p for p in predictions_list if 'predicted_demand' in p]),
        }
        ...
        if successful:
            ...
            actuals = [p.get('actual_demand') for p in successful if p.get('actual_demand') is not None]
            if actuals:
