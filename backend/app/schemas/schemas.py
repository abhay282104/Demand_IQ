...
class PredictionResponse(BaseModel):
    ...
    confidence_info: Optional[str] = Field(
        None, description="Confidence level description"
    )
    model_version: Optional[str] = Field(None, description="Model version used")
    model_metrics: Optional[Dict[str, float]] = Field(
        None, description="Current model metrics"
    )
    ...
    model_config = ConfigDict(
        ...
        json_schema_extra={
            "example": {
                ...
                "model_version": "1.0.0",
                "model_metrics": {"mae": 3.45, "rmse": 4.12, "r2": 0.89},
            }
        },
    )
...
class PredictionHistoryItem(BaseModel):
    ...
    confidence: float
    input_price: float
    input_promotion: bool
    input_holiday: bool
    input_economic_index: float
...
class TrainingResponse(BaseModel):
    ...
    metrics: Optional[Dict[str, Any]] = Field(
        None, description="Training metrics such as MAE/RMSE/R2"
    )
